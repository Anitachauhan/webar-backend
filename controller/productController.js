const { Fashion,Food,Machinery,Furniture, Sports } = require("../models/product");
...
//     if(req.query.id.length === 24 && req.query.industry.length !== 0 && (req.query.industry.localeCompare('SportsEquipment') === 0 || req.query.industry.localeCompare('SportsMerch') === 0 || req.query.industry.localeCompare('GOAT') === 0)) {
...
exports.scanQrCode = asyncHandler(async (req, res, next) => {
  ...
  else if (req.params["industry"].localeCompare("Sports") == 0) {
    Product = await Sports.findById(req.params["id"]);
  } 
...
exports.deleteProductById = asyncHandler(async (req, res, next) => {
  ...
  if (req.params["industry"].localeCompare("Fashion") == 0) {
    ...
    Products = await Sports.deleteOne({ _id: req.params["id"] });
  }
...
exports.getProductsByCategory = asyncHandler(async (req, res, next) => {
  ...
  if (req.params["industry"].localeCompare("Fashion") == 0) {
    ...
    Products = await Sports.find({ category: req.params["category"] });
  }
  res.status(200).json({
    Products,
  });
...
function getAllProductsInIndustry(industry) {
  if (industry.localeCompare("Fashion") == 0) {
    ...
    Products = Food.find();
  }else if (industry.localeCompare("Sports") == 0) {
    Products = Sports.find();
  }
  return Products;
}
